...
public class KuznechikFileCipher {
    ...
    // X function
    static private byte[] Kuz_X(byte[] a, byte[] b) {
        int i;
    ...
    // S function
    static private byte[] Kuz_S(byte[] in_data) {
        int i;
    ...
    // Galua mul
    static private byte Kuz_GF_mul(byte a, byte b) {
        byte c = 0;
        byte hi_bit;
        int i;
        for (i = 0; i < 8; i++) {
    ...
    // R function
    static private byte[] Kuz_R(byte[] state) {
        int i;
        byte a_15 = 0;
    ...
    //L shift
    static private byte[] Kuz_L(byte[] in_data) {
        int i;
        byte[] out_data;
    ...
    // function S^(-1)
    static private byte[] Kuz_reverse_S(byte[] in_data) {
        int i;
        byte[] out_data = new byte[in_data.length];
        for (i = 0; i < BLOCK_SIZE; i++) {
            int data = in_data[i];
    ...
    //function R^(-1)
    static private byte[] Kuz_reverse_R(byte[] state) {
        int i;
        byte a_0;
        a_0 = state[15];
        byte[] internal = new byte[16];
    ...
    //shift L^(-1)
    static private byte[] Kuz_reverse_L(byte[] in_data) {
        int i;
        byte[] out_data;
        byte[] internal;
        internal = in_data;
    ...
    // Transform F
    static private byte[][] Kuz_F(byte[] in_key_1, byte[] in_key_2, byte[] iter_const) {
        byte[] internal;
    ...
    public static void Kuz_Expand_Key(byte[] key_1, byte[] key_2) {
        int i;
    ...
    // Encode function
    public static byte[] Kuz_Encript(byte[] blk) {
        int i;
        byte[] out_blk;
        out_blk = blk;
        for (i = 0; i < 9; i++) {
    ...
    // Decode function
    public static byte[] Kuz_Decript(byte[] blk) {
        int i;
    ...
    public static byte[] addPadding(byte[] data) {
        ...
        byte[] paddedData = Arrays.copyOf(data, data.length + padLength);
    ...
    // ECB
    public static byte[] encryptECB(byte[] plainData) {
        byte[] paddedData = addPadding(plainData);
        ...
        for (int i = 0; i < paddedData.length; i += BLOCK_SIZE) {
            ...
            byte[] encryptedBlock = Kuz_Encript(block);
            System.arraycopy(encryptedBlock, 0, encryptedData, i, BLOCK_SIZE);
        }
        return encryptedData;
    }

    public static byte[] decryptECB(byte[] cipherData) {
        byte[] decryptedPaddedData = new byte[cipherData.length];
        for (int i = 0; i < cipherData.length; i += BLOCK_SIZE) {
            byte[] block = Arrays.copyOfRange(cipherData, i, i + BLOCK_SIZE);
            byte[] decryptedBlock = Kuz_Decript(block);
            System.arraycopy(decryptedBlock, 0, decryptedPaddedData, i, BLOCK_SIZE);
        }
        return removePadding(decryptedPaddedData);
    }

    public static byte[] getFileHash(String filePath, String algorithm) throws Exception {
        java.security.MessageDigest digest = java.security.MessageDigest.getInstance(algorithm);
        byte[] buffer = new byte[8192];
        int bytesRead;
        try (java.io.BufferedInputStream bis = new java.io.BufferedInputStream(java.nio.file.Files.newInputStream(java.nio.file.Paths.get(filePath)))) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            boolean useHash = fileSize > 1024 * 1024;
            byte[] contentToEncrypt;
            if (useHash) {
                System.out.println("File larger than 1 mb, using hash");
                contentToEncrypt = getFileHash(filePath, "SHA-256");
                System.out.println("Initial Hash:");
                System.out.println(DatatypeConverter.printHexBinary(contentToEncrypt));
            } else {
                System.out.println("File smaller than 1 mb, cipher content");
                contentToEncrypt = Files.readAllBytes(Paths.get(filePath));
            ...
            Kuz_Expand_Key(key1, key2);
            byte[] encryptedContent = encryptECB(contentToEncrypt);
            System.out.println("Encoded:");
            System.out.println(DatatypeConverter.printHexBinary(encryptedContent));
            byte[] decryptedContent = decryptECB(encryptedContent);
            if (useHash) {
                System.out.println("Decoded Hash:");
            } else {
                System.out.println("Decoded Content:");
            }
            System.out.println(DatatypeConverter.printHexBinary(decryptedContent));
        } catch (Exception e) {
